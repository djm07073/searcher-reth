...
fn main() -> eyre::Result<()> {
    ...
    let vault_address =
        config.relayer.vault_address.parse().map_err(|_| eyre!("Invalid vault address"))?;
    let signers = config.relayer.get_signers().unwrap();
    // database
    reth::cli::Cli::<EthereumChainSpecParser, SetupArgs>::parse().run(|builder, args| async move {
        ...
        let handle = builder
            ...
            .extend_rpc_modules(move |ctx| {
                let repository =
                    Arc::new(SearcherRepository::new(config.database.path.to_str().unwrap()));
            ...
            .install_exex("SearcherExEx", {
                move |ctx| {
                    let exex = SearcherExEx::exex(ctx, extension_for_exex, signers);
                    info!(
                        target: "reth-exex",
