        ...
        Ok(async move {
            ...
            while let Some(notification) = ctx.notifications.next().await {
                if let Ok(ExExNotification::ChainCommitted { new: chain }) = notification {
                    ...
                    // 1. find candidates to liquidate
                    // pending txs is not used for liquidatior strategy. pass empty vectors.
                    let profitable_candidates = strategy.find_profitable_candidates(
                        num_hash,
                        latest_state_provider,
                        Vec::<EthPooledTransaction>::new(),
                    )?;
                    ...
                    // 2. send candidates to relayer pool for braodcasting trnasactions
