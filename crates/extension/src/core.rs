...
pub struct SearcherExtension<'a, S: Strategy<'a>> {
    pub(crate) vault: Address,
    pub(crate) contract: Bytecode,
    pub(crate) max_profit_ratio: U256,
    pub(crate) min_profit_ratio: U256,
    pub(crate) candidates: Vec<HashMap<Address, Vec<Vec<<S as Strategy<'a>>::Action>>>>,
}
...
impl<'a, S: Strategy<'a>> SearcherExtension<'a, S> {
    ...
    pub fn update_candidates(&mut self, candidates: Vec<HashMap<Address, Vec<Vec<S::Action>>>>) {
