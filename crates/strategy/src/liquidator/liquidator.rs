...
impl ProcessorInfo {
    fn new(
        ...
        Self { table_name, processor_name: processor_name.to_string(), processor }
    }
}
...
impl Strategy for Liquidator {
    ...
    type Action = LiquidationPayload;
...
impl Liquidator {
    ...
    fn add_processor(&mut self, table_name: String, processor_name: String) {
        let table = match TableName::from_str(&table_name) {
            ...
            None => {
                return;
            } // Skip if table name is not recognized
        };

        let processor = match table {
            TableName::DolomiteBorrowPositions => {
                ProcessorInfo::new(table.as_str(), &processor_name, |block_data, db| {
                    Box::pin(process_dolomite_borrow_positions(block_data, db))
                })
            }
            TableName::AaveExecuteBorrow => {
                ProcessorInfo::new(table.as_str(), &processor_name, |block_data, db| {
                    Box::pin(process_aave_execute_borrow(block_data, db))
                })
            }
        };
    ...
    async fn process_block_data(
        ...
        if !event_results.is_empty() {
            let events_summary: Vec<String> = event_results
                .iter()
                .map(|(name, time)| format!("{}({}, {:.2}s)", name, 1, time.as_secs_f64()))
                .collect();
        ...
        if !failed_events.is_empty() {
            let failure_summary: Vec<String> =
                failed_events.iter().map(|(name, error)| format!("{}: {}", name, error)).collect();
