...
impl EthereumValue for alloy_primitives::Address {
    fn to_sql_value(&self) -> Box<dyn tokio_postgres::types::ToSql + Sync + Send> {
...
impl EthereumValue for Option<alloy_primitives::Address> {
    fn to_sql_value(&self) -> Box<dyn tokio_postgres::types::ToSql + Sync + Send> {
...
impl EthereumValue for Option<alloy_primitives::FixedBytes<32>> {
    fn to_sql_value(&self) -> Box<dyn tokio_postgres::types::ToSql + Sync + Send> {
        Box::new(self.as_ref().map(|bytes| bytes.to_string()))
    }
}

impl<'a> EthereumValue for Option<&'a alloy_primitives::FixedBytes<32>> {
    fn to_sql_value(&self) -> Box<dyn tokio_postgres::types::ToSql + Sync + Send> {
        Box::new(self.map(|bytes| bytes.to_string()))
    }
}

impl EthereumValue for Option<alloy_primitives::Uint<256, 4>> {
    fn to_sql_value(&self) -> Box<dyn tokio_postgres::types::ToSql + Sync + Send> {
        Box::new(self.as_ref().map(|u| u.to_string()))
    }
}
...
impl EthereumValue for Vec<u8> {
    fn to_sql_value(&self) -> Box<dyn tokio_postgres::types::ToSql + Sync + Send> {
        Box::new(alloy_primitives::hex::encode_prefixed(self))
    }
}
...
impl EthereumValue for alloy_primitives::FixedBytes<32> {
    fn to_sql_value(&self) -> Box<dyn tokio_postgres::types::ToSql + Sync + Send> {
        Box::new(self.to_string())
    }
}

impl<const BITS: usize, const LIMBS: usize> EthereumValue for alloy_primitives::Uint<BITS, LIMBS> {
    fn to_sql_value(&self) -> Box<dyn tokio_postgres::types::ToSql + Sync + Send> {
        Box::new(self.to_string())
    }
}

impl<const BITS: usize, const LIMBS: usize> EthereumValue for alloy_primitives::Signed<BITS, LIMBS> {
    fn to_sql_value(&self) -> Box<dyn tokio_postgres::types::ToSql + Sync + Send> {
        Box::new(self.to_string())
    }
}
